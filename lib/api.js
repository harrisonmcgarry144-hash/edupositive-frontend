...
async function request(method, path, body, opts = {}) {
  ...
  if (!res.ok) {
    ...
    if (data.requiresVerification && data.token) {
      if (typeof window !== "undefined") localStorage.setItem("ep_token", data.token);
    }
...
export const authApi = {
  ...
  verifyCode:     (code) => api.post("/api/auth/verify-code", { code }),
  resendCode:     () => api.post("/api/auth/resend-code"),
};
...
export const contentApi = {
  ...
  paragraphQuiz:      (lessonId, paragraph, index) => api.post(`/api/content/lessons/${lessonId}/paragraph-quiz`, { paragraph, paragraphIndex: index }),
...
export const usersApi = {
  ...
  studyTimeToday: () => api.get("/api/users/study-time/today"),
  logStudyTime:   (mins) => api.post("/api/users/study-time/log", { durationMins: mins }),
  readNotif:    (id) => api.put(`/api/users/me/notifications/${id}/read`),
  saveSubjectChoices: (b) => api.post("/api/users/me/subject-choices", b),
  mySubjects:   () => api.get("/api/users/me/subjects"),
...
export const classesApi = {
  ...
  join:               (b)      => api.post("/api/classes/join", b),
  invite:             (id, username) => api.post(`/api/classes/${id}/invite`, { username }),
  removeMember:       (id, uid) => api.delete(`/api/classes/${id}/members/${uid}`),
  assignments:        (id)     => api.get(`/api/classes/${id}/assignments`),
  createAssignment:   (id, b)  => api.post(`/api/classes/${id}/assignments`, b),
  completeAssignment: (id)     => api.post(`/api/classes/assignments/${id}/complete`),
  deleteAssignment:   (id)     => api.delete(`/api/classes/assignments/${id}`),
  leaderboard:        (id)     => api.get(`/api/classes/${id}/leaderboard`),
};

// ── Ranks ─────────────────────────────────────────────────────────────────────
export const ranksApi = {
  me:          () => api.get("/api/ranks/me"),
  leaderboard: () => api.get("/api/ranks/leaderboard"),
};

// ── Payments ──────────────────────────────────────────────────────────────────
export const paymentsApi = {
  status:         () => api.get("/api/payments/status"),
...
export const generateApi = {
  start:               (subjectId, board) => api.post(`/api/generate/start/${subjectId}/${board}`),
  status:              (subjectId, board) => api.get(`/api/generate/status/${subjectId}/${board}`),
  userSubjectsStatus:  () => api.get("/api/generate/user-subjects-status"),
};
